...
from .positionalembedding2d import positionalencoding2d
...
class CascadedPAM(nn.Module):
    def __init__(self, channels):
        ...
        self.stage1 = Trans(channels[0])
        self.stage2 = Trans(channels[1])
        self.stage3 = Trans(channels[2])

        # word_embedding in stage 1
        self.word_embed_1 = nn.Linear(128, 128, bias=False)

        # word_embedding in stage 2
        self.word_embed_2 = nn.Linear(96, 96, bias=False)
        self.word_merge_2 = nn.Linear(128 + 96, 96, bias=False)

        # word_embedding in stage 3
        self.word_embed_3 = nn.Linear(64, 64, bias=False)
        self.word_merge_3 = nn.Linear(96 + 64, 64, bias=False)

        self.pos_enc_1 = PositionEncodingSine(channels[0], stride=4)
        self.pos_enc_2 = PositionEncodingSine(channels[1], stride=2)
        self.pos_enc_3 = PositionEncodingSine(channels[2], stride=1)
    ...
    def forward(self, fea_left, fea_right):
        ...
        fea_left_s2 = fea_left_s2.permute(0, 2, 3, 1)  # (n, h, w, c)
        fea_right_s2 = fea_right_s2.permute(0, 2, 3, 1)
        fea_left_s3 = fea_left_s3.permute(0, 2, 3, 1)
        fea_right_s3 = fea_right_s3.permute(0, 2, 3, 1)  # (n, h, w, c)

        # word embedding
        fea_left_s1 = self.word_embed_1(fea_left_s1)
        fea_right_s1 = self.word_embed_1(fea_right_s1)
        ...
        # stage 2: 1/8
        fea_left = F.interpolate(fea_left.permute(0, 3, 1, 2), scale_factor=2, mode='bilinear').permute(0, 2, 3, 1)
        fea_right = F.interpolate(fea_right.permute(0, 3, 1, 2), scale_factor=2, mode='bilinear').permute(0, 2, 3, 1)
        # word embedding
        fea_left_s2 = self.word_embed_2(fea_left_s2)
        fea_right_s2 = self.word_embed_2(fea_right_s2)
        ...
        fea_right = F.interpolate(fea_right.permute(0, 3, 1, 2), scale_factor=2, mode='bilinear').permute(0, 2, 3, 1)
        # word embedding
        fea_left_s3 = self.word_embed_3(fea_left_s3)
        fea_right_s3 = self.word_embed_3(fea_right_s3)
        ...
        fea_right = self.pos_enc_3(fea_right)
